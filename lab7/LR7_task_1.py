...
class CityMap:
    def __init__(self, distancesMatrix, number_of_cities):
        self.distances = distancesMatrix
        self.number_of_cities = number_of_cities
        self.pheromones = [[np.random.rand() for j in range(number_of_cities)] for i in range(number_of_cities)]
...
class Colony:
    ...
    def FindRoute(self, city_map, city_number):
        min_distance = float('inf')
        route = []
        
        for cycle in range(self.max_colony_cycles):
            pheromones_delta = [[0.0 for i in range(city_map.number_of_cities)] for j in range(city_map.number_of_cities)]
            
            for ant_number in range(self.number_of_ants):
                ant = Ant(city_number)
                
                while len(ant.visited_cities) < city_map.number_of_cities:
                    next_city = self.GetNextCity(ant, city_map)
                    ant.Move(next_city, city_map.distances[ant.current_city][next_city])
                
                ant_distance = ant.distance + city_map.distances[ant.current_city][ant.starting_city]
                
                if ant_distance < min_distance:
                    min_distance = ant_distance
                    route = ant.visited_cities
                
                for city in range(len(ant.visited_cities) - 1):
                    pheromones_delta[ant.visited_cities[city]][ant.visited_cities[city + 1]] += self.pheromone_addition / ant_distance

            city_map.UpdatePheromones(self.pheromone_evaporation_rate, pheromones_delta)
        
        return min_distance, route
    
    # Формування списку ймовірностей переміщення в місто для мурахи
    def GetProbabilities(self, ant, city_map):
        result = [0 for i in range(city_map.number_of_cities)]
        total_probability = 0
        
        for new_city in range(city_map.number_of_cities):
            if (new_city != ant.current_city) and (self.ant_can_visit_previous_cities or new_city not in ant.visited_cities):
                probability = pow(city_map.pheromones[ant.current_city][new_city], self.pheromone_importance) * pow(1 / city_map.distances[ant.current_city][new_city], self.distance_importance)
                result[new_city] = probability
                total_probability += probability
        
        result = [result[i] / total_probability for i in range(city_map.number_of_cities)]
        
        return result
    
    # Вибір наступного міста для мурахи
    def GetNextCity(self, ant, city_map):
        probabilities = self.GetProbabilities(ant, city_map)
...
city_map = CityMap(distance, len(distance[0]))
colony = Colony(len(distance[0]))
result = colony.FindRoute(city_map, 18)

print(f"Отриманий найкоротший шлях: {result[0]} км")

# Вивід отриманого маршруту
city_routes = "Отриманий маршрут: "

for i in result[1]:
    city_routes += cities[i]
    if i != result[1][-1]:
        city_routes += "->"
